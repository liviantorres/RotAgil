...
const ModalEditarRota = ({ onClose, onSave, pontos, route }) => {
  const [ponto1, setPonto1] = useState(route.initialDeliveryPointId || '');
  const [ponto2, setPonto2] = useState(route.destinationDeliveryPointId || '');
  const [distancia, setDistancia] = useState(route.distance || '');
  const { trajetoId } = useParams();

  useEffect(() => {
    setPonto1(route.initialDeliveryPointId);
    setPonto2(route.destinationDeliveryPointId);
    setDistancia(route.distance);
  }, [route]);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/route/${route.id}`, {
        roadId: trajetoId,  
      ...
      if (response.status === 200) {
        ...
        console.error('Falha ao editar a rota', response.statusText);
      }
    } catch (error) {
  ...
  return (
    <ModalOverlay>
      <ModalContent>
        ...
        <ModalHeader>Editar Rota</ModalHeader>
...
export default ModalEditarRota;
